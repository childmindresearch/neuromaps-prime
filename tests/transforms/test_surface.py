...
def touch_inputs(paths: dict[str, str | Path], skip: set[str]) -> None:
...
class TestSurfaceSphereProjectUnproject:
    ...
    def test_missing_inputs(
        self, mock_paths: dict[str, str | Path], missing: str, msg: str
    ):
    ...
    def test_success(self, mock_paths: dict[str, str | Path]):
    ...
    def test_missing_output(self, mock_paths: dict[str, str | Path]):
...
class TestMetricResample:
    ...
    def test_invalid_resample_method(self, mock_paths: dict[str, str | Path]):
        ...
        with pytest.raises(NotImplementedError, match="not implemented"):
            metric_resample(**mock_paths, method="invalid")

    @pytest.mark.parametrize("method", ["ADAP_BARY_AREA", "BARYCENTRIC"])
    def test_success(self, mock_paths: dict[str, str | Path], method: str):
        """Test successful call."""
        ...
        with run_patched(func_path, side_effect=lambda **_: produce()) as mock_wb:
            metric_resample(**mock_paths, method=method)
...
class TestLabelResample:
    ...
    def test_invalid_resample_method(self, mock_paths: dict[str, str | Path]):
        ...
        touch_inputs(mock_paths, skip={"output_file_path", "area_surfs"})
        with pytest.raises(NotImplementedError, match="not implemented"):
            label_resample(**mock_paths, method="invalid")
    ...
    def test_success(self, mock_paths: dict[str, str | Path], method: str):
        ...
        with run_patched(func_path, side_effect=lambda **_: produce()) as mock_wb:
            label_resample(**mock_paths, method=method)
            mock_wb.assert_called_once()
            assert Path(mock_paths["output_file_path"]).exists()
...
class TestSurfaceTransformIntegration:
    ...
    def test_surface_sphere_project_unproject(
        ...
        assert utils.get_vertex_count(sphere_in) == utils.get_vertex_count(
            result.sphere_out
        )
...
def test_surface_to_surface_transformer(
    ...
    if transformer_type == "metric":
        assert utils.estimate_surface_density(output.metric_out) == target_density
    elif transformer_type == "label":
        assert utils.estimate_surface_density(output.label_out) == target_density
